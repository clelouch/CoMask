...
class MFPN(nn.Module):
    ...
    Example:
        ...
        >>> self = MFPN(in_channels, 11, len(in_channels)).eval()
    ...
    def __init__(self, in_channels, out_channels, num_outs, start_level=0, end_level=-1, branch=1, attention='RCA',
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            fpn_conv = SIM(branch=self.branch, in_channel=out_channels, out_channel=out_channels,
                           attention=self.attention, conv_cfg=conv_cfg, norm_cfg=norm_cfg, activation=self.activation)
        ...
        if add_extra_convs and extra_levels >= 1:
            for i in range(extra_levels):
                ...
                extra_fpn_conv = nn.Sequential(
                    ConvModule(in_channels, out_channels, 3, stride=2, padding=1, conv_cfg=conv_cfg,
                               norm_cfg=norm_cfg, activation=self.activation, inplace=False),
                    SIM(branch=self.branch, in_channel=out_channels, out_channel=out_channels,
                        attention=self.attention, conv_cfg=conv_cfg, norm_cfg=norm_cfg, activation=self.activation))
