...
class D2DetHead(nn.Module):
    ...
    def __init__(self, num_convs=8, roi_feat_size=14, in_channels=256, conv_kernel_size=3,
                 class_agnostic=False, conv_cfg=None, norm_cfg=dict(type='GN', num_groups=36),
                 MASK_ON=False):
        ...
        for i in range(self.num_convs):
            ...
            self.convs.append(
                ConvModule(in_channels, self.conv_out_channels, self.conv_kernel_size, stride=stride,
                           padding=padding, conv_cfg=self.conv_cfg, norm_cfg=self.norm_cfg, bias=False))
    ...
    def forward(self, x, idx=None):
        ...
        x0 = self.convs(x)
        x_m = self.D2Det_mask(x0)  # 论文中预测有对象的区域
    ...
    def get_target(self, sampling_results):
        ...
        for j in range(map_size):
            ...
            for i in range(map_size):
                ...
                # 这里的x, y表示当前的(j, i)点在原图像中的坐标
                dx = (pos_bboxes[:, 2] - pos_bboxes[:, 0]) / (map_size - 1)

                # target表示gt_box相对于当前坐标的左上右下的偏离
                ...
                # 按照论文中的设置，只有在gt_box内部的坐标点的预测被保留
    ...
    def get_target_mask(self, sampling_results, gt_masks, rcnn_train_cfg):
        # 这个函数首先针对所有的positive samples，获取了它们的bbox，然后根据对应的gt_masks和gt_class
        # 从gt_mask中截取bbox范围的部分，并resize到mask_size的大小
        ...
        instances = mask_target([res.pos_bboxes for res in sampling_results], pos_assigned_gt_inds, gt_masks,
                                rcnn_train_cfg)
        # 转换成mask大小，这里的masks是论文中的用于预测regression的部分
        masks = F.interpolate(instances.unsqueeze(0), scale_factor=1 / 4, mode='bilinear', align_corners=False). \
            squeeze(0)
    ...
    def get_bboxes_avg(self, det_bboxes, D2Det_pred, D2Det_pred_mask, img_meta):
        ...
        # 由于det_bboxes是(n, 4)矩阵，其中分别表示左上右下的坐标，因此，这里分别计算了h(高度), w(宽度), y(映射到原图上的纵坐标),
        # x(映射到原图上的横坐标)
        ...
        # target: (n, 7 * 7, 4), 对应的一个预测上特征图每个位置预测的左上右下四个坐标
    ...
    def get_target_maskiou(self, sampling_results, gt_masks, mask_pred, mask_targets, sample_idx):
        ...
        # the whole instance
        # 这里是首先计算了proposal的bbox范围内的gt_area的面积，再计算了整个instance mask的面积
        # 再获取了这俩个的比例
        ...
        # 由于ratio = gt_area_in_bbox / gt_area_whole_instance
        # 通过mask_target / ratio 可以获取整个的面积
        gt_full_areas = mask_targets.sum((-1, -2)) / (area_ratios + 1e-7)

        # 这里的mask_iou_targets中，分子是proposal内的mask与预测值的交集
        # 而分母是预测值与whole instance area的并集再减去分子，由于proposal的mask一定是
        # whole instance mask的子集，所以这里的真实含义就是pred mask与真实的whole mask的IoU值
