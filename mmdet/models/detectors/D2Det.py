...
class D2Det(TwoStageDetector):
    def __init__(self, backbone, rpn_head, bbox_roi_extractor, bbox_head, reg_roi_extractor,
                 D2Det_head, train_cfg, test_cfg, neck=None, shared_head=None, pretrained=None):
    ...
    def forward_train(self, img, img_meta, gt_bboxes, gt_labels, gt_bboxes_ignore=None, gt_masks=None,
        ...
        if self.with_bbox:
            ...
            # classification branch, in this branch, the bbox is not predicted, thus is None
            # 注意，deformable roi pooling只用在了classification支路,
            # self.bbox_roi_extractor.num_inputs = [4, 8, 16, 32]
            ...
            reg_feats = self.reg_roi_extractor(x[:self.reg_roi_extractor.num_inputs], pos_rois)
            ...
            if self.MASK_ON == False:
                ...
                # x1_1, x2_1, y1_1, y2_1分别表示gt_box的左右上下边界
                ...
                # 计算CrossEntropyLoss损失，注意use_sigmoid是True，所以其实是CrossEntropyWithLogit
                # 这里的reg_mask_pred就是个7x7的矩阵，表示的论文中
                # 有效地区域，这一有效区域也就是目标检测时pred_box和gt_box的交集
                ...
                # reg_points: 包含四个通道，分别是x, y坐标以及预测bbox的宽度和高度
                # reg_targets: 包含四个通道，分别是当前(x, y)坐标到gt_bbox的左右上下距离
                # reg_masks: 将instances缩小到原来的1/4大小之后，用0.5的阈值进行二值化
                # reg_instances: 在gt_mask中截取bbox范围内的instance mask
                reg_points, reg_targets, reg_masks, reg_instances = \
                    self.D2Det_head.get_target_mask(sampling_results, gt_masks, self.train_cfg.rcnn)
                ...
                # 计算instance mask的binary_cross_entropy_with_logits
                loss_instance = self.loss_roi_instance(reg_instances_pred, reg_instances, pos_gt_labels)

                # 计算预测的mask和whole instance mask的交并比，注意这里的作用其实就是mask scoring rcnn，reg_iou_targets
                # 就是预测的mask与真值mask的iou值。
                reg_iou_targets = self.D2Det_head.get_target_maskiou(sampling_results, gt_masks, reg_instances_pred[
                ...
                losses.update(
                    dict(loss_reg=loss_reg, loss_mask=loss_mask, loss_instance=loss_instance, loss_iou=loss_iou))
                #############################################
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        # 由于bbox_head的bbox reg没有工作，这里的det_bboxes就是rpn网络输出的proposal，再经过nms而得
        # 注意这里的det_bboxes是(n, 5), 其中第5列表示对应预测类别的得分，这里的simple_test_bboxes就是对应的bbox_head
        # 并且，在bbox_head.py中的get_det_bboxes函数返回的是基于零的label值，由于以往0表示背景，这里相当于去掉了原来的0
        # 再将所有的class label - 1，这也就是下面的label在用的时候都要+1的原因
        ...
        if det_bboxes.shape[0] != 0:
            ...
            det_bboxes = self.D2Det_head.get_bboxes_avg(det_bboxes, reg_pred, reg_pred_mask, img_meta)
