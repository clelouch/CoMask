...
def bbox2roi_expand(bbox_list):
    ...
    for img_id, bboxes in enumerate(bbox_list):
        if bboxes.size(0) > 0:
            ...
            x1 = bboxes[:, 0] - (bboxes[:, 2] - bboxes[:, 0]) / 2
            y1 = bboxes[:, 1] - (bboxes[:, 3] - bboxes[:, 1]) / 2
            x2 = bboxes[:, 2] + (bboxes[:, 2] - bboxes[:, 0]) / 2
            y2 = bboxes[:, 3] + (bboxes[:, 3] - bboxes[:, 1]) / 2
